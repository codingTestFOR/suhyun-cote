...
3+1 = 4분이 걸리게 된다. 3번 사람은 1번, 2번 사람이 돈을 뽑을 때까지 기다려야 하기 때문에, 총 3+1+4 = 8분이 필요하게 된다. 4번 사람은 3+1+4+3 = 11분, 5번 사람은 3+1+4+3+2 = 13분이 걸리게 된다. 
...
기다리는 시간을 최소화하기 위해 인출 시간이 짧은 사람부터 먼저 줄을 세우기!

문제 풀이 흐름
인출 시간이 짧은 순서대로 오름차순 정렬
각 사람이 기다리는 시간은 자기 앞에 있는 사람들의 인출 시간을 모두 더한 값.
각 사람이 기다리는 시간들의 합을 구하면 최솟값이 됨 . 

"""

# 사람 수 N과 각 사람의 인출 시간 Pi 입력
N = int(input())
...
for time in P:
    ...
    total_time += current_sum  # 전체 합에 더해줌
